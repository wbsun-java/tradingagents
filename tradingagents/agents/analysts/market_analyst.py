...
"""
            + f"""The stock's O'Neil base-pattern setup has already been deterministically read for you below -- do not call any tool to re-derive it. Use `primary_pattern.pattern_type` to identify the winning pattern, then narrate its dated and priced geometry, status, and evidence. Every pattern's geometry names the base's starting peak -- the peak that ended the prior advance (`base_start` for the double bottom; `start_date`/`range_high` or `left_high` for flat bases and cups) -- and the report must name that starting peak's date and price when describing the base; for a forming cup, also state the recovery progress from `retrace_pct`. Mention `other_detections` in at most one sentence. For a double bottom, state `second_low_behavior` explicitly: `undercut` is a brief shakeout of the first low, `equal` means the floor was retested and held, and `higher` means the second low held above the first (aggressive institutional accumulation); narrate it with both lows' dates and prices. Apply this three-tier precedence rule when synthesizing the report's overall technical conclusion: (1) if the Wyckoff `phase_bias` is bullish or bearish, it remains the final direction as already stated above; (2) if Wyckoff is neutral and this JSON's `setup_bias` is bullish, `setup_bias` becomes the directional anchor instead -- chart-pattern, trend-template, and indicator evidence may only adjust conviction within that direction, not flip it to the opposite direction; if Wyckoff is instead non-neutral and conflicts with this JSON's direction, say so explicitly ("conflicts with the O'Neil base-pattern structure") but still lead with the Wyckoff direction; (3) if both Wyckoff and this JSON are neutral, weigh the remaining technical evidence normally. When the O'Neil double-bottom base and the chart-pattern tool's generic double bottom cover the same lows, describe them as a single structure qualifying under O'Neil's stricter criteria at O'Neil's tier -- never as two independent patterns confirming each other. Do not invent patterns beyond what this JSON reports, and do not eyeball structures from the raw CSV. The JSON's `canslim_earnings` block carries a deterministic read of O'Neil's C (current quarterly EPS growth, year-over-year) and A (annual EPS growth) letters: state each letter's verdict with its numbers and dates from the `evidence` strings, mention `acceleration` when it is not null, and treat these as fundamental context for the base-pattern read -- they never change the technical `setup_bias` or the precedence rules above. When a verdict is `unavailable`, say so and why; never infer earnings growth from any other data.
...
Write a very detailed and nuanced report of the trends you observe. Provide specific, actionable insights with supporting evidence to help traders make informed decisions."""
            + """ Make sure to append a Markdown table at the end of the report to organize key points in the report, organized and easy to read, including a row for the Wyckoff phase, phase_bias, dominant_weight, and invalidated flag, and a separate row for the O'Neil `pattern_type`, `status`, `setup_bias`, and `secondary_weight`."""
            + get_language_instruction()
        )
